<x-dashboard.layout :title="__('label.edit_role')">
    ...
    <section id="multiple-column-form">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">{{ __('label.edit_role') }}</h4>
            </div>
                                            ...
                                            @foreach ($permissions as $permission)
                                                <label class="switch" for="{{ $permission->id }}">
                                                    ...
                                                    <input type="checkbox" class="items" value="{{ $permission->id }}"
                                                        @checked(in_array($permission->id, old('permissions', $role_permission_ids ?? []))) name="permissions[]"
                                                        id="{{ $permission->id }}">
                                                </label>
                                            @endforeach
                                        </div>

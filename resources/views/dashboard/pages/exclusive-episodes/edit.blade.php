<x-dashboard.layout :title="__('label.edit_exclusive_episode')">
            ...
            <div class="breadcrumbs-top">
                ...
                <div class="breadcrumb-wrapper">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item">
                            {{ __('label.edit_exclusive_episode') }}
                        </li>
                    </ol>
                </div>
            </div>
    ...
    <section id="multiple-column-form">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">{{ __('label.edit_exclusive_episode') }}</h4>
            </div>
            <form action="{{ route('admin.exclusive-episodes.update', $episode) }}" method="POST"
                enctype="multipart/form-data" id="episode_form">
                @csrf
                @method('PUT')
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6 col-12">
                            <div class="mb-2">
                                ...
                                <select name="user_id" id="user_id" class="select2 form-select" required>
                                    <option value="" disabled>{{ __('label.select_podcaster') }}</option>
                                    @foreach ($podcasters as $podcaster)
                                        <option value="{{ $podcaster->id }}" @selected(old('user_id', $episode->user_id) == $podcaster->id)
                        ...
                        <div class="col-md-6 col-12">
                            <div class="mb-2">
                                ...
                                <select name="channel_id" id="channel_id" class="select2 form-select" required>
                                    <option value="" disabled>{{ __('label.select_channel') }}</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="col-md-6 col-12">
                            <div class="mb-2">
                                ...
                                <select name="season_id" id="season_id" class="select2 form-select" required>
                                    <option value="" disabled>{{ __('label.select_season') }}</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="col-md-6 col-12 d-flex align-items-center gap-2 parent">
                            ...
                            <img src="{{ displayFile($episode->image) }}" id="image_preview" class="preview rounded"
                                width="60px" />
                        </div>
                        ...
                        <div class="col-md-12 col-12">
                            <div class="mb-2">
                                ...
                                <textarea rows="4" id="description" name="description" class="form-control"
                                    placeholder="{{ __('label.description') }}" required>{{ old('description', $episode->description) }}</textarea>
                            </div>
                        </div>
                        ...
                        <div class="col-md-6 col-12">
                            <div class="mb-2">
                                ...
                                <select name="media_type" id="media_type" class="form-select" required>
                                    <option value="" disabled>{{ __('label.select_type') }}</option>
                                    <option value="video" @selected(old('media_type', $episode->media_type) == 'video')>{{ __('label.video') }}</option>
                                    <option value="audio" @selected(old('media_type', $episode->media_type) == 'audio')>{{ __('label.audio') }}</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="col-md-6 col-12">
                            <div class="mb-2">
                                ...
                                <select name="media_source" id="media_source" class="form-select" required>
                                    <option value="" disabled>{{ __('label.select_type') }}</option>
                                    <option value="link" @selected(old('media_source', $episode->media_source) == 'link')>{{ __('label.link') }}</option>
                                    <option value="fileupload" @selected(old('media_source', $episode->media_source) == 'fileupload')>{{ __('label.file') }}</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="col-md-12 col-12" id="link_field" style="display:none;">
                            <div class="mb-2">
                                ...
                                <input type="url" id="link" name="link" value="{{ old('link', $episode->link) }}"
                        ...
                        <div class="col-md-12 col-12" id="file_field" style="display:none;">
                            <div class="mb-2">
                                ...
                                @if ($episode->media_source === 'fileupload')
                                    <small class="text-muted">{{ __('label.current_file') }}: <a
                                            href="{{ displayFile($episode->link) }}" target="_blank">{{ __('label.view') }}</a></small>
                                @endif
                            </div>
                        </div>
                    </div>
    ...
    @push('js')
        ...
        <script>
            $(document).ready(function() {
                // Load initial channels and seasons
                const podcasterId = $('#user_id').val();
                const currentChannelId = {{ $episode->channel_id ?? 'null' }};
                const currentSeasonId = {{ $episode->season_id ?? 'null' }};
                
                if (podcasterId) {
                    loadChannelsAndSeasons(podcasterId, currentChannelId, currentSeasonId);
                }

                // Load channels and seasons when podcaster is selected
                $('#user_id').on('change', function() {
                    const podcasterId = $(this).val();
                    loadChannelsAndSeasons(podcasterId);
                });

                function loadChannelsAndSeasons(podcasterId, selectedChannelId = null, selectedSeasonId = null) {
                    const channelSelect = $('#channel_id');
                    const seasonSelect = $('#season_id');

                    channelSelect.empty().append('<option value="" disabled>{{ __('label.select_channel') }}</option>');
                    seasonSelect.empty().append('<option value="" disabled>{{ __('label.select_season') }}</option>');
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                ...
                                if (selectedChannelId) {
                                    channelSelect.val(selectedChannelId).trigger('change');
                                }
                                if (selectedSeasonId) {
                                    seasonSelect.val(selectedSeasonId).trigger('change');
                                }
                            },
                            error: function(xhr) {
                ...
                $('#media_source').on('change', function() {
                    ...
                    if (mediaSource === 'link') {
                        ...
                        // If current value is a file path (not a URL), clear it
                        if (currentLinkValue && !currentLinkValue.startsWith('http://') && !currentLinkValue.startsWith('https://')) {
                            $('#link').val('');
                        }
                        ...
                        // If current value is a URL (not a file path), clear it
                        if (currentLinkValue && (currentLinkValue.startsWith('http://') || currentLinkValue.startsWith('https://'))) {
                            $('#link').val('');
                        }
                        
                        // Show file field (optional in edit mode - file already exists)
                        $('#file_field').show();
                ...
                if (currentMediaSource === 'link') {
                    ...
                    $('#file_field').show();
                    $('#link').prop('required', false);
                    $('#media_file').prop('required', false); // Optional in edit mode
